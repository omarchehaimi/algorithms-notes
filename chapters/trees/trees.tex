...
For both ways the average complexity in case of a search is \(O(\vert E \vert + \vert V \vert)\), where \(\vert E \vert\) and \(\vert V \vert\) are the number of edges and the number of vertex respectively. If all the tree is traversed the complexity is \(O(\vert V \vert)\). Breadth-first search is less space-efficient than depth-first search because BFS keeps a priority queue of the entire frontier while DFS maintains a few pointers at each level. More details on this are in the appendix \ref{binappendix} about the full implementation of the two methods. 

\subsection{Depth-first search}
In the depth-first search there are several different ways to perform a search on a tree. Example of applications of this search are for solving mazes and scheduling problems.

\paragraph{Pre-order search}
\label{preorder}
In the \textbf{pre-order search} the first node to be checked as visited is the root. The following node is the left child by convention. Once checked the left child the process is repeated until the first node without any children is reached. At that point the same process is applied to the right side: all the nodes on the right are checked until all the nodes are checked as visited \ref{trees_5} (Root-Left-Right). 
...
\label{inorder}
In the \textbf{in-order search} the first node to be checked is the first node without children on the left side (or right side). Once checked off this node the next one to be checked off is its parent, and the process is repeated again on the right side of the nodes, until all the nodes are checked off (Left-Root-Right).
