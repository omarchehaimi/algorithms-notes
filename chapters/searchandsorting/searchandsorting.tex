...
There is a more efficient way to search an element in an array sorted in ascending way. This method is the \textbf{binary search} \cite{wikibinarysearch} (\href{https://en.wikipedia.org/wiki/Binary_search_algorithm}{Binary Search Algorithm, Wikipedia}).
...
\section{Bubble Sort}
\textbf{Bubble sort} is the easiest sorting algorithm working on arrays. Bubble sort works by swapping two elements at each step if they are in the wrong order, and it repeats this process until all the array is not completely ordered \cite{wikibubblesort} (\href{https://en.wikipedia.org/wiki/Bubble_sort}{Bubble Sort, Wikipedia}). In this way the bigger elements tend to move at the bottom of the array, like bubbles that move at the top of a water bottle.
