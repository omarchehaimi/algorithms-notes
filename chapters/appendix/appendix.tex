...
\begin{algorithm}[H]
	...
	\LinesNumbered
  	...
  	\Fn{\FRecursivePreorder{$node$}}{
		...
		visit(node)\;
		Recursive-Preorder(node.left)\;
		Recursive-Preorder(node.right)\;
  	}
  	\;
...
\begin{algorithm}[H]
	...
	\LinesNumbered
  	...
  	\Fn{\FRecursivePostorder{$node$}}{
		...
		Recursive-Postorder(node.left)\;
		Recursive-Postorder(node.right)\;
		visit(node)\;
  	}
  	\;
...
\caption{Post-order iterative implementation pseudocode.}
\end{algorithm}
