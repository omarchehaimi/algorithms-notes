...
\begin{table}[H]
	...
	\begin{tabular}{c | c c c c c c}
		...
		\hline
		a & \mybox[rounded corners=6pt, line width=1pt, draw=black, fill=green!25]{mycol}{0_{a}}\tikzmark{a} & \(8_{a}\) & \(2_{a}\) & \(5_{a}\) & \(\infty\) & \(\infty\) & \(\infty\) & \(\infty\) \\

	\end{tabular}

	\begin{tikzpicture}[overlay, remember picture]
		\draw [->, line width=0.3mm] ({pic cs:a}) -- ({pic cs:b});
		\draw [->, line width=0.3mm] ({pic cs:bb}) -- ({pic cs:cc});
		\draw [->, line width=0.3mm] ({pic cs:c}) -- ({pic cs:d});
	\end{tikzpicture}
\end{table}