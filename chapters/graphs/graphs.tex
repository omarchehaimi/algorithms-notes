...
The \textbf{shorted path problem} is the problem of finding a path between two nodes such that the sum of the weights is minimized. In case of an undirected graph the shortest path is the path that pass through the minimum number of nodes \citep{wikishortestpath} (\href{https://en.wikipedia.org/wiki/Hamiltonian_path}{Shortest path problem, Wikipedia}).  
